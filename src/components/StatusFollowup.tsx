...
const StatusFollowupChart = () => {
  const data: {
    ...
    Docusign: number;
    AQ: number;
    "Deal Conversion Rate (%)": string;
    "DS Conversion Rate (%)": string;
  ...
  const groupedData = data.reduce((acc, cur) => {
    const status = cur.Status as keyof typeof acc;
    if (!acc[status]) {
      acc[status] = {
        status,
        seniorAMRatio: status === "Senior AM" ? cur.followUpRatio : 0,
        officialRatio: status === "Official" ? cur.followUpRatio : 0,
        gpRatio: status === "GP" ? cur.followUpRatio : 0,
        count: 1,
      };
    } else {
      acc[status].count += 1;
      if (status === "Senior AM")
        acc[status].seniorAMRatio += cur.followUpRatio;
      if (status === "Official") acc[status].officialRatio += cur.followUpRatio;
      if (status === "GP") acc[status].gpRatio += cur.followUpRatio;
    }
    return acc;
  }, {} as Record<string, { status: string; seniorAMRatio: number; officialRatio: number; gpRatio: number; count: number }>);

  const processedData = Object.values(groupedData).reduce((acc, cur) => {
    return {
      ...acc,
      seniorAMRatio: acc.seniorAMRatio || cur.seniorAMRatio / cur.count,
      officialRatio: acc.officialRatio || cur.officialRatio / cur.count,
      gpRatio: acc.gpRatio || cur.gpRatio / cur.count,
    };
  }, {} as any);

  return (
    <div className="w-full">
      <h2 className="text-[24px] text-center mb-4">Status별 Follow-up Ratio</h2>
      <div className="h-96">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Legend />
            <Bar dataKey="seniorAMRatio" fill="#8884d8" name="Senior AM" />
            <Bar dataKey="officialRatio" fill="#82ca9d" name="Official" />
            <Bar dataKey="gpRatio" fill="#ffc658" name="GP" />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export default StatusFollowupChart;
