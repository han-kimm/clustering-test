...
const TeamPerformance = () => {
  ...
  rawData.forEach((item) => {
    ...
    teamData[item.SF].count += 1;
    teamData[item.SF].DMC += item.DMC;
    teamData[item.SF].Deal += item.Deal;
    teamData[item.SF].Docusign += item.Docusign;
    teamData[item.SF].followUpRatio += item.followUpRatio;
    teamData[item.SF].DealConvRate += parseFloat(
      item["Deal Conversion Rate (%)"].replace("%", "")
    );
    teamData[item.SF].DSConvRate += parseFloat(
      item["DS Conversion Rate (%)"].replace("%", "")
    );
  });

  // 평균 계산
  const processedData = Object.entries(teamData).map(([team, values]) => ({
    team,
    DMC: values.DMC / values.count,
    Deal: values.Deal / values.count,
    Docusign: values.Docusign / values.count,
    followUpRatio: values.followUpRatio / values.count,
    DealConvRate: values.DealConvRate / values.count,
    DSConvRate: values.DSConvRate / values.count,
  }));
  ...
  return (
    <div className="w-full">
      <div className="space-y-8">
        {/* 실적 차트 */}
        <div>
          <h3 className="text-[24px] text-center mb-4">
            팀별 평균 실적 & Follow-up Ratio
          </h3>
          <div className="h-96">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={processedData}>
                ...
                <Tooltip />
                <Legend />
                <Bar yAxisId="left" dataKey="DMC" name="DMC" fill="#8884d8" />
                <Bar yAxisId="left" dataKey="Deal" name="Deal" fill="#82ca9d" />
                <Bar
                  yAxisId="left"
                  dataKey="Docusign"
                  name="Docusign"
          ...
          <div className="h-96">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={processedData}>
                ...
                <Legend />
                <Bar
                  dataKey="DealConvRate"
                  name="Deal 전환율 (%)"
                  fill="#8884d8"
                />
                <Bar dataKey="DSConvRate" name="DS 전환율 (%)" fill="#82ca9d" />
                <Line
                  type="monotone"
                  dataKey="followUpRatio"
                  name="Follow-up Ratio (%)"
